...
from library.logging_pack_20200906 import *
...
class simulator_func_mysql:
    ...
    def variable_setting(self):
        ...
        if self.simul_num in (1, 4):
            # 시뮬레이팅 시작 일자(분 별 시뮬레이션의 경우 최근 1년 치 데이터만 있기 때문에 start_date 조정 필요(daliy_craw))
            ...
            if self.simul_num == 4:
                self.simul_start_date = "20200424"
                # 분별 시뮬레이션을 사용하고 싶을 경우 아래 옵션을 True로 변경하여 사용
                self.use_min = True
                # 아침 9시에만 매수를 하고 싶은 경우 True, 9시가 아니어도 매수를 하고 싶은 경우 False(분별 시뮬레이션 적용 가능 / 일별 시뮬레이션은 9시에만 매수, 매도)
                self.only_nine_buy = True
    ...
    ##!@수정####################################################################################################################################################################################
    # 매수 할 종목의 리스트를 선정 알고리즘 (아침 9시에 실행) 중요!!
    ...
    def get_sell_list(self, i):
        ...
        if self.sell_list_num == 1:
            ...
            sell_list = self.engine_simulator.execute(sql % (0, self.sell_point, self.losscut_point)).fetchall()    #fetchall은 리스트형태로 데이터를 가져옴
